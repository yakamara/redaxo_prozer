						...
						var uploader = new qq.FileUploader({
							...
							onComplete: function(id, fileName, result) {
								...
								if(result.clipdata.id) {
					    			link = "'.pz::url('screen', 'clipboard', 'get', ['mode' => 'image_src_raw', 'image_size' => 'xl', 'image_type' => 'image/jpg']).'"+"&clip_id="+result.clipdata.id;
					    			$.post(link, "", function(data) {
					    				if(data != "") {
					    					pz_hidden.val("PHOTO;ENCODING=b;TYPE=JPEG:"+data);

											link = "'.pz::url('screen', 'clipboard', 'get', ['mode' => 'image_inline', 'image_size' => 'm']).'"+"&clip_id="+result.clipdata.id;
