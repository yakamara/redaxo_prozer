...
class pz_user_screen
{
    ...
    {
        $edit_link = pz::url($p['mediaview'], $p['controll'], $p['function'], ['user_id' => $this->user->getId(), 'mode' => 'edit_user']);
        $del_link = pz::url($p['mediaview'], $p['controll'], $p['function'], ['user_id' => $this->user->getId(), 'mode' => 'delete_user']);
    ...
    public static function getTableListView($users, $p = [])
    {
    ...
    public function getProjectPermTableView($p = [], $project)
    {
        ...
        if ($project->hasEmails() == 1) {
            $status = $projectuser->hasEmails() ? $status = 1 : $status = 0;
        }
        $td[] = $project_user_screen->getPermTableCellView('emails', $status);

        $status = 2;
        if ($project->hasCalendarEvents() == 1) {
            $status = $projectuser->hasCalendarEvents() ? $status = 1 : $status = 0;
        }
        ...
        $status = 2;
        if ($project->hasCalendarEvents() == 1) {
            $status = $projectuser->hasCalDAVEvents() ? $status = 1 : $status = 0;
        }
        $td[] = $project_user_screen->getPermTableCellView('caldav_events', $status);

        $status = 2;
        if ($project->hasCalendarJobs() == 1) {
            $status = $projectuser->hasCalDAVJobs() ? $status = 1 : $status = 0;
        }
        $td[] = $project_user_screen->getPermTableCellView('caldav_jobs', $status);

        $status = 2;
        if ($project->hasFiles() == 1) {
            $status = $projectuser->hasFiles() ? $status = 1 : $status = 0;
        }
    ...
    public function getEditForm($p = [])
    {
        ...
        if ($xform->getObjectparams('actions_executed')) {
            ...
            if ($projectsadmin == 1) {
                ...
                $this->user->removePerm('projectsadmin');
            }
    ...
    public function getMyEditForm($p = [])
    {
        $header = '
        <header>
          <div class="header">
    ...
    public function getMyPasswordEditForm($p = [])
    {
        $header = '
        <header>
          <div class="header">
