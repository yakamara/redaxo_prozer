...
class pz_calendar_rule_event extends pz_calendar_event
{
    ...
    protected function __construct(pz_calendar_rule $rule, array $params = [], $exception = false)
    {
    ...
    public function save($mode = self::THIS)
    {
        ...
        switch ($mode) {
            ...
            case self::FUTURE:
                $nBase = parent::create();
                foreach (['project_id', 'title', 'location', 'description', 'url', 'from', 'to'] as $key) {
                ...
                foreach (['Frequence', 'Interval', 'Days', 'Months', 'WeekDays', 'Nth', 'End', 'Count'] as $key) {
                ...
                $nExcpt = $nExcpt == $exceptions ? [] : $nExcpt;
    ...
    public function delete($mode = self::THIS)
    {
        ...
        switch ($mode) {
            case self::ALL:
                self::_delete('e.rule_id = ?', [$this->rule->getId()]);
                $this->rule->delete();
                break;
    ...
    public static function create(pz_calendar_rule $rule = null, DateTime $baseFrom = null)
    {
        $event = new self($rule);
        $event->new = true;
    ...
    public static function getByBaseFrom(pz_calendar_rule $rule, DateTime $baseFrom)
    {
        $base = $rule->getBase();
    ...
    public static function getAllRuleEvents($rule, DateTime $from, DateTime $to = null)
    {
        $from->setTime(0, 0);
        $to = $to ?: clone $from;
        $to->setTime(23, 59, 59);
    ...
    private static function nthWeekDay($nth, $weekDay)
    {
        static $nths = ['next', 'first', 'second', 'third', 'fourth', pz_calendar_rule::LAST => 'last', pz_calendar_rule::PREVIOUS => 'previous'];
        static $weekDays = ['day', 'mon', 'tue', 'wed', 'thu', 'fri', 'sat', 'sun'];
        return $nths[$nth] . ' ' . $weekDays[$weekDay];
    }

    public static function nextIndex(array $array, $item)
    {
        $count = count($array);
