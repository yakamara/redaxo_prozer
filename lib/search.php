...
{
    public $search_items = []; // Treffer
    public $search_order = '';
    public $search_link_search_vars = [];
    public $search_vars = [];
    public $search_url = '';
    ...
    public $search_fulltext_fields = ['fulltext' => 1,'name' => 5]; // fieldname => relevance
    public $search_order_fields = ['id_new' => 'id desc', 'id_old' => 'id asc'];

    public function pz_search()
    {
    ...
    public function setSearchTable($search_table)
    {
    ...
    public function setSearchLinkvars($k, $v)
    {
    ...
    public function setSearchVars($label, $k, $v, $how = '=')
    {
        $this->search_vars[$label] = ['key' => $k,'value' => $v,'how' => $how];
    }

    public function setSearchListAmount($la = 5)
    {
        $this->search_list_amount = $la;
    }

    public function getSearchListAmount()
    {
        return $this->search_list_amount;
    }

    public function setSearchPaginateField($pf)
    {
    ...
    public function setSearchOrderKey($sort = '')
    {
    ...
    public function getSearchOrderKey()
    {
        return $this->search_order_key;
    }

    public function executeSearch($pos = 0)
    {
        ...
        foreach ($this->search_vars as $label => $v) {
            ...
            switch ($v['how']) {
                ...
                case('OR_EXPLODE'):
                    $q = [];
                    foreach (explode(',', $v['value']) as $sv) {
                        $q[] = '`'.$v['key'].'` LIKE \''.mysql_real_escape_string($sv).'%\'';
                    }
                    $singlequery = '('.implode(' OR ', $q).')';
                    break;
                case('OR_EXPLODE='):
                    $q = [];
                    foreach (explode(',', $v['value']) as $sv) {
                        $q[] = '`'.$v['key'].'` = \''.mysql_real_escape_string($sv).'\'';
                    }
                    $singlequery = '('.implode(' OR ', $q).')';
                    break;
                default:
    ...
    */
    ...
    public function getSearchPaginatePlainView($p = [])
    {
        if (!isset($p['skip_key'])) {
            $p['skip_key'] = 'skip';
        }
        ...
        if ($current > $this->search_counter_all || $current < 0) {
            $current = 0;
        }
        ...
        if ($next >= $this->search_counter_all) {
            $next = '';
        }
