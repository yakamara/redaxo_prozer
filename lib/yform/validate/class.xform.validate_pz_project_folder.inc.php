...
class rex_yform_validate_pz_project_folder extends rex_yform_validate_abstract
{
                ...
                if ($o->getName() == $field) {
                    ...
                    if ($current_parent_id == 0) {
                        ...
                        if (!($folder = pz_project_file::get($current_parent_id)) || !$folder->isDirectory()) {
                            ...
                            $this->params['warning_messages'][$o->getId()] = pz_i18n::msg('error_folder_name_parent');

                        // gibt es parentfolder in diesem projekt
                        } elseif ($folder->getProjectId() != $project_id) {
                            $this->params['warning'][$o->getId()] = $this->params['error_class'];
                            $this->params['warning_messages'][$o->getId()] = pz_i18n::msg('error_folder_project_id');
                        } else {
                            $parents = $folder->getParentsIds();
                            $parents[] = $folder->getId();
                            if (in_array($folder_id, $parents)) {
                                $this->params['warning'][$o->getId()] = $this->params['error_class'];
                                $this->params['warning_messages'][$o->getId()] = pz_i18n::msg('error_folder_in_same_folder');
                            }
                        }
                    }
                }
