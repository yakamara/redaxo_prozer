...
class pz_eml
{
    ...
    public function __construct($src, $part_id = 0, $depth = 0)
    {
        $this->src = $src;
        $this->parent = null;
    ...
    */
    ...
    public static function parseHeaderToArray($header)
    {
        ...
        if (preg_match("#\nReply-to: (.*)$#im", $header, $regs)) {
            $return['reply-to'] = strtolower(trim($regs[1]));
        }
        if (preg_match('#\nErrors-To:(.*)#im', $header, $regs)) {
            $return['errors-to'] = strtolower(trim($regs[1]));
        }
        if (preg_match('#\nReturn-path:(.*)#im', $header, $regs)) {
            $return['return-path'] = strtolower(trim($regs[1]));
        }
        ...
        if (preg_match("#\ndate: (.*)$#im", $header, $regs)) {
            $return['date'] = trim($regs[1]);
        }

        if (preg_match('#\ncontent-transfer-encoding: (.*)$#im', $header, $regs)) {
            $return['content-transfer-encoding'] = strtolower(trim($regs[1]));
        }
        if (preg_match('#\nmime-version:(.*)#im', $header, $regs)) {
            $return['mime-version'] = strtolower(trim(str_replace(['>', '<'], '', $regs[1])));
        }
        ...
        if (preg_match('#\nContent-ID:(.*)#im', $header, $regs)) {
            $return['content-id'] = trim(str_replace(['>', '<'], '', $regs[1]));
        }
        if (preg_match('#\nMessage-ID:(.*)#im', $header, $regs)) {
            $return['message-id'] = trim(str_replace(['>', '<'], '', $regs[1]));
        }
        if (preg_match('#\nImportance:(.*)#im', $header, $regs)) {
            $return['importance'] = trim($regs[1]);
        }
        if (preg_match('#\nSensitivity:(.*)#im', $header, $regs)) {
            $return['sensitivity'] = trim($regs[1]);
        }

        // X Fields
        if (preg_match('#\nX-Priority:(.*)#im', $header, $regs)) {
            $return['x-priority'] = trim($regs[1]);
        }
        if (preg_match('#\nx-mailer:(.*)$#im', $header, $regs)) {
            $return['x-mailer'] = trim($regs[1]);
        }
        if (preg_match('#\nX-Provags-ID:(.*)#im', $header, $regs)) {
            $return['x-provags-id'] = trim($regs[1]);
        }
        ...
        if ($content_type_all = pz_eml::parseHeaderType($header, 'Content-Type')) {
            ...
            if (preg_match('#method[ ]*=[" \']*([a-zA-Z0-9\-]*)[" \';,]*#im', $content_type_all, $regs)) {
    ...
    public static function parseHeaderType($header = '', $type = '', $decodeCharset = true)
    {
        preg_match("#\n".$type.":(.*)((\n(\t| ))*(.*))*#im", $header, $type);
    ...
    public static function decodeCharset($value)
    {
    ...
    public static function parseAddressList($address)
    {
