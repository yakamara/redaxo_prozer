...
class pz_email_account_screen{
	...
	function getListView($p = array())
	{
    	...
    	$edit_link = "javascript:pz_loadPage('email_account_form','".pz::url("screen","emails","setup",array_merge($p["linkvars"],array("mode"=>"edit_email_account","email_account_id"=>$this->email_account->getId())))."')";
    
    	$del_link = "javascript:pz_loadPage('email_accounts_list','".pz::url("screen","emails","setup",array_merge($p["linkvars"],array("mode"=>"delete_email_account","email_account_id"=>$this->email_account->getId())))."')";
    
    	$info = "";
    	if($this->email_account->getVar("last_login") != "")
 		   	$info = rex_i18n::msg("emailaccount_last_login").': '.$this->email_account->getVar("last_login");
    	if($this->email_account->getVar("login_failed") == 1)
    		$info .= " | ".rex_i18n::msg("emailaccount_last_login_failed");
    	elseif($info != "")
    		$info .= " | ".rex_i18n::msg("emailaccount_last_login_ok");
          ...
          <article>
            <header>
              <a class="detail clearfix" href="'.$edit_link.'">
                <hgroup>
                  <h3 class="hl7"><span class="title">'.htmlspecialchars($this->email_account->getVar("name")).' / '.htmlspecialchars($this->email_account->getVar("email")).'</span></h3>'.$info.'
                </hgroup>
            ...
            <footer>
              ...
              <a class="bt2" href="'.$edit_link.'">'.rex_i18n::msg("edit").'</a>
            </footer>
          </article>
        ';
	...
	static function getAccountsListView($email_accounts,$p)
	{
		...
		return '<div id="email_accounts_list" class="design2col">'.$f->parse('pz_screen_list').'</div>';
	}
	...
	public function getEditForm($p = array()) 
	{
		...
		$xform->setObjectparams("form_action", "javascript:pz_loadFormPage('email_account_edit','email_account_edit_form','".pz::url('screen','emails','setup',array("mode"=>'edit_email_account'))."')");
		...
		if($xform->getObjectparams("actions_executed")) {
			...
			$return .= pz_screen::getJSUpdateLayer('email_accounts_list',pz::url('screen','emails','setup',array("mode"=>'list')));
		}else
		{
	...
	static function getAddForm($p = array()) 
	{
		...
		$xform->setObjectparams("form_action", "javascript:pz_loadFormPage('email_account_add','email_account_add_form','".pz::url('screen','emails','setup',array("mode"=>'add_email_account'))."')");
