...
class pz_email_account extends pz_model
{
    ...
    public static function getAccounts($user_id = '', $status = '')
    {
        $where = [];
        $params = [];
    ...
    public static function getAsArray($user_id = '')
    {
        $return = [];
            ...
            if ($emails !== false) {
                ...
                foreach ($emails as $email) {
                    ...
                    if ($email_header != '') {
                        ...
                        if (isset($email_header_array['to'])) {
                            $email->setTo($email_header_array['to']);
                        }
                        if (isset($email_header_array['cc'])) {
                            $email->setCc($email_header_array['cc']);
                        }
                        if (isset($email_header_array['from'])) {
                            $email->setFrom($email_header_array['from']);
                        }

                        if (isset($email_header_array['to_emails'])) {
                            $email->setVar('to_emails', $email_header_array['to_emails']);
                        }
                        if (isset($email_header_array['cc_emails'])) {
                            $email->setVar('cc_emails', $email_header_array['cc_emails']);
                        }
                        if (isset($email_header_array['from_emails'])) {
                            $email->setVar('from_emails', $email_header_array['from_emails']);
                        }

                        if (isset($email_header_array['subject'])) {
                            $email->setSubject($email_header_array['subject']);
                        }
                        if (isset($email_header_array['content-type'])) {
                            $email->setContentType($email_header_array['content-type']);
                        }
                        if (isset($email_header_array['date'])) {
                            $email->setDate($email_header_array['date']);
                        }
                        if (isset($email_header_array['reply-to'])) {
                            $email->setReplyTo($email_header_array['reply-to']);
                        }
                        if (isset($email_header_array['importance'])) {
                            $email->setImportance($email_header_array['importance']);
                        }
